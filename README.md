...
3. iPhone 用 Safari 走到「跟師傅走一圈」那一步，按「在 iPhone 開啟鏡頭建圖」。Launch 檢視器提供 hit-test、anchors、DOM overlay；不提供平面偵測與深度。

### 操作流程

打開網站就是師傅來看房（`/`）。一次問一題：先問這是公寓／大樓的一戶、樓中樓還是透天（決定要看幾層）；大門朝哪用手機羅盤直接量（畫面先告訴你站哪裡）；再問誰當家、還有誰、房子幾年蓋的，問完先揭曉宅卦、財位、文昌位與命卦合不合。接著建圖：手機支援 AR（Android Chrome、iPhone 經 App Clip）就拿鏡頭跟師傅走一圈，點外牆轉角、房間角落、門窗與家具的位置，朝向由牆面自動判斷；不支援就站在門口描一下房子、在草圖上塗出房間、每間房點家具靠的那段牆。多層住家逐層建。然後每間房回一段評語，最後給分數、最要緊的三件事與今年要避的方位。每題都有台階（不知道、先跳過、用畫的）。

看完之後有三個地方：

1. 「怎麼做」：白話摘要＋由簡到繁的行動清單；「進階分析」有分數與八宅、飛星、流年、形勢、五行分頁，以及各派有別的判法設定（取向依據、灶位判法、兼向門檻、替卦）；「分享圖」產出單層分析圖，「下載」產出完整報告長圖。
2. 「平面圖」：微調工具。補門窗、拖家具、拍照描圖（拍建商平面圖，兩點校正比例後描）、多樓層；要重建就按「跟師傅重新建圖」回到引導。
3. 「更多」：羅盤量向、屋外環境問卷（路沖、壁刀、高壓電塔）、知識庫、隱私與免責。
