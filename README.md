...
| 輸出 | 總評與分項分數、優先處理清單、九宮疊圖、分享用 PNG（平面圖＋分析層）、Markdown 匯出、列印 |
...
2. 「平面圖」：三種來源擇一。空間掃描（Android Chrome 或 iPhone App Clip）沿房子走一圈點外牆轉角、各房間角落與門窗，自動生成；拍照描圖（拍建商平面圖，兩點校正比例後描）；或手繪。之後放床、灶、桌等物件並用「依大門朝向校正北方」。多樓層在「更多」面板新增。
3. 「屋外環境」：勾選路沖、壁刀、高壓電塔等問卷。
4. 「報告」：總評與八宅、飛星、流年、形勢、五行分頁，可匯出 Markdown 或列印。
