...
最短路徑是首頁的「跟師傅走一遍」（`/start`）：師傅到門口，一次問一題（大門朝哪：畫面先告訴你站在門裡面朝外、手機平放，直接用羅盤量；再問誰當家、還有誰、房子幾年蓋的），問完先揭曉宅卦、財位、文昌位與命卦合不合；接著問主臥在哪一邊、床頭朝哪，每間房回一段評語，想看幾間就看幾間；最後給分數、最要緊的三件事與今年要避的方位。每題都有台階（用羅盤量、不知道、先跳過），想看門沖、樑壓等位置關係再畫平面圖。完整流程如下：
