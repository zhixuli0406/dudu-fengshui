...
### Added
- 「師傅來看房」引導模式取代新手村（`/start`，首頁主入口）：一題一畫面、師傅口吻發問、每題底下一行「為什麼要問」、每題都有台階（羅盤量、不知道屋齡、這間先跳過）。開場是師傅到門口的短場景（打字機對話、二選一、可跳過、只演一次）；問完大門、當家的人、家人、屋齡後先揭曉宅卦、財位、文昌與命卦合不合；主臥方位與床頭朝向之後每間房回一段評語；「還有哪一間」迴圈到使用者喊停，總結給分數、最要緊三件事與今年五黃二黑，再導向清單、3D 看房或平面圖精靈。大門這一題直接用羅盤量（畫面上先畫出站位：站在大門裡面、面朝屋外、手機平放機頂對門外），即時讀數、穩定度提示、一顆「就是這個方向」；沒有感測器或量不到時才退回八方向手選。流程狀態機在 `src/guide/script.ts`（有測試），進度與待問房間持久化於 store。參考觀察記錄在 `docs/research/06-guided-flow-reference.md`。
