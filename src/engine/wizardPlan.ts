...
/**
 * Outline from size, rooms from painted cells, main door on its wall, key furniture against the chosen wall.
 * `northOffset` is fixed so the main door faces `facingBearing` (the compass reading taken in the doorway).
 * Ids are deterministic (`wz_r0`, `wz_door`, …) so the same answers always give the same plan.
 */
export function deriveWizardPlan(w: WizardInput, facingBearing: number): DerivedPlan {
  ...
  const mainDoor: Item = { id: 'wz_door', ...doorOnWall(outline, w.doorWall, w.doorT) }
  const northOffset = (((facingBearing - ((mainDoor.facing ?? 0) + 180)) % 360) + 360) % 360
  const center = polygonCentroid(outline)
  const items: Item[] = [mainDoor]
  ...
  const plan: FloorPlan = { ...emptyPlan('1F', 0), outline, rooms, items, northOffset, gridCm: 50 }
  return { outline, cells, rooms, mainDoor, items, plan }
}
